...
export function InsightsPanel({ insights, title = "What to Know" }: InsightsPanelProps) {
  ...
  const getIcon = (type: Insight['type']) => {
    switch (type) {
      case 'alert':
        return (
          <div className="flex-shrink-0 w-6 h-6 rounded-full bg-accent-orange-light flex items-center justify-center">
      ...
      default:
        return (
          <div className="flex-shrink-0 w-6 h-6 rounded-full bg-primary-blue-lighter flex items-center justify-center">
  ...
  const getBorderColor = (type: Insight['type']) => {
    switch (type) {
      ...
      case 'warning':
        return 'border-l-accent-orange/60'
      default:
